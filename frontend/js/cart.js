...
// 페이지 로드 시 상품 정보를 가져와서 표시
window.addEventListener("DOMContentLoaded", () => {
  getProduct(5); // 상품 ID 정보를 가져옵니다.
});

let productData; // 전역 변수로 상품 정보를 담을 변수 선언
...
async function getProduct(productId) {
  ...
  productData = data; // 가져온 상품 정보를 전역 변수에 저장
  displayProduct(productData); // 상품 정보를 화면에 표시하는 함수 호출
}
...
function placeOrder() {
  // 여기에 주문 처리 로직을 추가하세요.
  // 주문 정보를 서버에 전송하는 코드 등을 작성하면 됩니다.
  alert("등록이 완료되었습니다!");
}
